...
//Promise抛出的错误必须设置catch方法来捕获 不然就不会显示到外面
const promise = new Promise(function (resolve, reject) {
    throw new Error('test');
});
promise.catch(function (error) {
...
//Promise的状态一旦resolved之后再抛出错误就不会被捕获了 因为状态凝固了
const promise = new Promise(function (resolve, reject) {
    resolve('ok');
    throw new Error('test');
});
...
//Promise对象的错误具有冒泡性质 会一直向后传递 直到被捕获位置 错误总是会被下一个catch语句捕获 

//一般来说不要在then里面定义reject状态的回调函数 总是在catch里面设置

//还可以用一个catch来捕获另一个catch抛出的错误
someAsyncThing().then(function () {
    return someOtherAsyncThing();
}).catch(function (error) {
    console.log('oh no', error);
